...
const TelemetryCharts = ({ telemetry, selectedDriver, leaderDriver, currentTime }) => {
  ...
  const chartData = useMemo(() => {
    ...
    const leaderData = leaderDriver ? telemetry.filter(d => d.Driver === leaderDriver && d.Time >= startTime && d.Time <= endTime) : [];

    // Merge data by Time (approximate matching)
    // Since we resampled to 1s, we can match exactly on integer seconds?
    // Actually Time is float.
    
    // Let's just map the driverData and try to find matching leader point
    return driverData.map(d => {
        const l = leaderData.find(ld => Math.abs(ld.Time - d.Time) < 0.5);
        return {
            time: d.Time,
            dist: d.Distance,
            speed: d.Speed,
            ...
            rpm: d.RPM,
            gear: d.nGear,
            leaderSpeed: l ? l.Speed : null,
            leaderThrottle: l ? l.Throttle : null,
            leaderBrake: l ? l.Brake : null,
            leaderRPM: l ? l.RPM : null,
            leaderGear: l ? l.nGear : null,
        };
    });
  }, [telemetry, selectedDriver, leaderDriver, currentTime]);
    ...
    <div className="grid grid-cols-3 gap-2 bg-gray-900/80 p-2 rounded border border-gray-700 backdrop-blur-sm h-full">
      <div className="col-span-3 flex justify-between items-center border-b border-gray-700 pb-1 mb-1">
        <h3 className="text-white text-[10px] font-bold uppercase tracking-widest">
            Telemetry: <span className="text-rbr-red">{selectedDriver}</span> vs <span className="text-gray-400">{leaderDriver || 'Leader'}</span>
        </h3>
      ...
      <div className="h-full min-h-[150px] w-full relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Line type="monotone" dataKey="leaderSpeed" stroke="#666" strokeWidth={1} strokeDasharray="2 2" dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="h-full min-h-[150px] w-full relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Line type="step" dataKey="leaderThrottle" stroke="#666" strokeWidth={1} strokeDasharray="2 2" dot={false} isAnimationActive={false} />
